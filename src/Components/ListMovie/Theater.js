...
export default function Theater() {
    ...
    const getDataBanner = async () => {
        try {
            let getData = await Axios(`http://www.omdbapi.com/?apikey=f0d99cd&s=action&type=movie&page=1`)
            if (movieList.length === 0) setMovieList(getData.data.Search)
        } catch (error) { console.log(error) }
    }; getDataBanner()

    return(
        <div className="col-12 theater mb-4">
                ...
                >
                    {
                        movieList.map((e, i) => {
                            return (
                                <div className="col-12 col-sm-12 col-md-12 col-lg-12 list-movie-box" key={ i }>
                                    <a href="/">
